    ...
    def recv(self, frame):
        ...
        if self.detector is None:
            self.detector = ExamDetector(
                use_yolo=st.session_state.get('use_yolo', YOLO_AVAILABLE),
                enable_sound=st.session_state.get('enable_sound', True)
            )
        annotated = self.detector.process(img)
...
def main():
    ...
    **Instant Detection:**
    ...
    if start and not st.session_state.monitoring:
        ...
        st.session_state.use_yolo = use_yolo_checkbox
    ...
    if st.session_state.monitoring:
        ...
        if webrtc_ctx.video_processor:
            detector = webrtc_ctx.video_processor.detector
            if detector and detector.violations:
                viol_text = '<div class="violation-box">' + ''.join([
                    f'<p><strong>#{len(detector.violations)-i}</strong> {v["type"].title()} at {v["time"]}</p>'
                    for i, v in enumerate(reversed(detector.violations))
                ]) + '</div>'
            else:
